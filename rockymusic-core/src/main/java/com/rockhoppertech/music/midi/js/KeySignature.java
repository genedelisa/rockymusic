...
public class KeySignature implements Serializable {
   ...
   // private Scale scale;
    ...
    private KeySignature(int majorMinor, int sf) {
        this.mm = (byte) majorMinor;
        this.sf = (byte) sf;
        this.displayName = getKeySignatureName(this.mm, this.sf);
        KeySignature.names.put(this.displayName, this);
        //TODO this needs to be the name as a key, i.e. cs and df
        this.root = getKeySignatureRootName(this.mm, this.sf);
        this.root = this.root.trim();
        if (logger.isDebugEnabled()) {
//            String s = String.format("root '%s'for %d %d", this.root, this.mm, this.sf);
//            logger.debug(s);
        }
        if (MAJOR == mm) {
            majorRoots.put(this.root, this);
...
//        }
    ...
    public String toString() {
        ...
        sb.append(" sf=").append(this.sf);
        return sb.toString();
    }

    public void addToTrack(Track track, long tick) {
        MetaMessage mm = this.getMIDIKeySignature();
...
//    }
    ...
    public Pitch getRoot() {
        return KeySignature.getKeySignatureRoot(this.mm, this.sf);
    }
