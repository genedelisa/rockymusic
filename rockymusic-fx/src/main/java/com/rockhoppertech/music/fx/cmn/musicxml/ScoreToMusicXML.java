 ...
 * 
 */
public class ScoreToMusicXML {
    private static final Logger logger = LoggerFactory
            .getLogger(ScoreToMusicXML.class);

    public static void main(String[] args) {
        Score score = new Score();
        score.setKeySignatureAtBeat(1d, KeySignature.CMAJOR);
        score.setTimeSignatureAtBeat(1d, 4, 4);
...
//        track.addKeySignatureAtBeat(1d, KeySignature.EFMAJOR);
        ...
        try {
            OutputStream os = new FileOutputStream("testmusicxml-score.xml");
            emitXML(score, os, "Grand Opus", "Giacomo");
    ...
    public static void emitXML(Score score, OutputStream out,
            String workTitle, String composer)
            throws IOException,
            XMLStreamException {
        
        resolution = score.getResolution();
        ...
        int partnum = 1;
        writer.add(createNewLine(eventFactory));
        for (MIDITrack track : score) {
            String partID = "P" + partnum++;
            writer.add(eventFactory.createStartElement("", "", "score-part"));
            writer.add(eventFactory.createAttribute("id", partID));
            writer.add(createNewLine(eventFactory));
            ...
            writer.add(eventFactory.createStartElement(
                    ...
                    "",
                    "score-instrument"));
            writer.add(eventFactory.createAttribute("id", partID +"-I1"));
            writer.add(createNewLine(eventFactory));
            ...
            createNode(eventFactory, writer, "virtual-name", track
                    .getInstrument()
                    .getName());
            writer.add(eventFactory.createEndElement(
                    "",
            ...
            writer.add(eventFactory
                    .createEndElement("", "", "score-instrument"));
            writer.add(createNewLine(eventFactory));
            //
        ...
        partnum = 1;
        for (MIDITrack track : score) {
            emitXML(track, out, "P" + partnum++, outputFactory, writer, eventFactory);
        }
